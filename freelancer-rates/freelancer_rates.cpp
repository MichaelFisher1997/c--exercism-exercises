...
int monthly_rate(double hourly_rate, double discount) {
    ...
    // discount.
    float monthly = daily_rate(hourly_rate)*22;
    return (int)std::ceil(apply_discount(monthly, discount));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double disDay = apply_discount(daily_rate(hourly_rate), discount); 
    return (int)std::floor(budget/disDay);
}